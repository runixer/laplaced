...
### Added
- The LongMemEval harness now provides stage-level evidence recall, immutable ingestion snapshots, dataset-relative dates, official judging, offline summaries, and paired run comparisons.
...
- Memory extraction now retries once when the model returns broken JSON, instead of silently dropping that conversation's memory updates.
