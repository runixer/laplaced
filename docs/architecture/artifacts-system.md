...
**Исключение — аудио/видео рядом с живым голосовым (v0.11.1).** Если в текущем сообщении
есть собственное аудио или видео, выбранные reranker'ом голосовые/аудио/видео-артефакты
**не** прикладываются байтами (`ArtifactLoader.LoadWithOptions{SkipAudioVideo}`): модель,
получив две аудиодорожки, транскрибирует обе и склеивает их в одну цитату — пятиминутная
запись из памяти «звучит» в конце сегодняшнего двухминутного голосового. Summary такого
файла всё равно попадает в `<artifact_context>`; байты вернутся на текстовом ходе, когда
reranker выберет его снова. Срабатывание видно в TraceQL:
`{ span.laplace.artifacts_recall_skipped_audio > 0 }`.
