...
- Инструмент не предлагается, когда ему нечего загрузить: при нулевом числе топиков-кандидатов
  (только люди/артефакты) первый и единственный запрос сразу идёт с `json_object` без `tools`.
  Если раунд запросил пустой список или только ID вне пула кандидатов, exploration считается
  исчерпанным и следующий запрос — финальный, без `tools` (атрибут спана
  `reranker.exploration_exhausted`; `forced_finalization` по-прежнему означает «упёрлись в кап»).
  Причина: `tools` вместе с `json_object` заставляли flash-lite звать инструмент почти всегда —
  с пустыми или выдуманными ID — вплоть до капа, то есть 4 LLM-вызова на каждый tool-using ход.
