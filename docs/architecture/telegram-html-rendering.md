...
> **Контекст транспортов (v0.10.0+):** рендеринг живёт за интерфейсом `Renderer`,
> привязанным к транспорту (см. [transports.md](./transports.md)). Описанное здесь
> — Telegram-реализация (Markdown → HTML, лимит 4096 UTF-16, конвертация LaTeX в
> Unicode). У Mattermost/Time свой рендерер: Markdown отдаётся как есть (сервер
> рендерит сам, включая KaTeX), а разбиение делается по числу рун. Возможности
> транспорта объявляются в `Capabilities` (`ParseMode`, `SupportsLatex`,
> `MaxMessageLen`), так что код выше по стеку не ветвится по имени транспорта.
