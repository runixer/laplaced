...
func TestBasicAuthMiddleware(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "Auth Enabled - API Route - No Credentials",
			authEnabled:    true,
			authUsername:   "admin",
			authPassword:   "secret",
			requestPath:    "/api/agent-log/1",
			expectedStatus: http.StatusUnauthorized,
		},
		{
			name:           "Auth Enabled - API Route - Correct Credentials",
			authEnabled:    true,
			authUsername:   "admin",
			authPassword:   "secret",
			requestPath:    "/api/agent-log/1",
		...
		{
			name:           "Auth Enabled - Healthz Is Public",
			authEnabled:    true,
			authUsername:   "admin",
			authPassword:   "secret",
			requestPath:    "/healthz",
		...
		{
			name:           "Auth Enabled - Metrics Is Public",
			authEnabled:    true,
			authUsername:   "admin",
			authPassword:   "secret",
			requestPath:    "/metrics",
		...
		{
			name:           "Auth Enabled - Webhook Is Public",
			authEnabled:    true,
			authUsername:   "admin",
			authPassword:   "secret",
			requestPath:    "/telegram/hook-secret",
			expectedStatus: http.StatusOK,
		},
	}
