...
func TestIncomingFromMattermost_Mapping(t *testing.T) {
	...
	t.Run("top-level DM threads under its own post id", func(t *testing.T) {
		ev := mattermost.PostedEvent{
			Post:        mattermost.Post{ID: "post1", UserID: "user1", ChannelID: "chan1", RootID: "", Message: "hello"},
			ChannelType: "D",
		}
	...
	t.Run("reply preserves existing root and non-DM channel", func(t *testing.T) {
		ev := mattermost.PostedEvent{
