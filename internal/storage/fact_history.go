...
func (s *Store) AddFactHistory(h FactHistory) error {
	...
	query := `
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`
	_, err := s.exec(query, h.FactID, h.UserID, h.Action, h.OldContent, h.NewContent, h.Reason, h.Category, h.Relation, h.Importance, h.TopicID, h.RequestInput, s.dialect.BindTime(h.CreatedAt))
	return err
}
...
func (s *Store) UpdateFactHistoryTopic(oldTopicID, newTopicID int64) error {
	...
	_, err := s.exec(query, newTopicID, oldTopicID)
	return err
}
