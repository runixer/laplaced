...
func setArtifactCreatedAt(t *testing.T, store *Store, artifactID int64, offset time.Duration) {
...
func setArtifactLastFailedAt(t *testing.T, store *Store, artifactID int64, offset time.Duration) {
...
func setArtifactRetryCount(t *testing.T, store *Store, artifactID int64, retryCount int) {
...
func addSessionMessage(t *testing.T, store *Store, userID ScopeID, topicID *int64) int64 {
	t.Helper()
...
func TestGetSessionArtifacts(t *testing.T) {
	...
	t.Run("rejects empty userID", func(t *testing.T) {
		_, err := store.GetSessionArtifacts(ctx, "", 10, 24*time.Hour)
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "UserID required")
	})
