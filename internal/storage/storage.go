...
// ArtifactReferenceSource records why an existing artifact was associated
// with an assistant reply. It is intentionally bounded: callers may either
// persist an artifact generated by the current turn or one selected from the
// user's trusted stored-artifact inventory.
...
// OutboundArtifactReference is one ordered, transport-neutral artifact
// presentation attached to a logical assistant reply. Repeating ArtifactID is
// valid (for example preview followed by the byte-exact original); Ordinal is
...
// PersistOutboundArtifacts separates provenance ownership from repeated use.
// OwnedArtifactIDs are newly-created rows whose temporary message_id=0 should
// become the new history row. References associate delivered artifacts with
// the reply without changing their canonical artifacts.message_id owner.
type PersistOutboundArtifacts struct {
	OwnedArtifactIDs []int64
	References       []OutboundArtifactReference
}

// HistoryArtifactReference is the persisted M:N association returned in
// delivery order.
