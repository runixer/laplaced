...
func (s *Store) AddStat(stat Stat) error {
	...
	_, err := s.exec(query, stat.UserID, stat.TokensUsed, stat.CostUSD)
	return err
}
...
func (s *Store) GetDashboardStats(userID ScopeID) (*DashboardStats, error) {
	...
	// 1. Topics Stats
	// Total Topics
	err := s.queryRow("SELECT COUNT(*) FROM topics"+whereUser, args...).Scan(&stats.TotalTopics)
	if err != nil {
	...
	if stats.TotalTopics > 0 {
		// Avg Size
		err = s.queryRow("SELECT AVG(end_msg_id - start_msg_id + 1) FROM topics"+whereUser, args...).Scan(&stats.AvgTopicSize)
		if err != nil {
		...
		whereProcessed := " WHERE facts_extracted = " + s.dialect.BoolLit(true)
		if userID != "" {
			whereProcessed += " AND user_id = ?"
		}
		err = s.queryRow("SELECT COUNT(*) FROM topics"+whereProcessed, args...).Scan(&processedCount)
		if err != nil {
		...
		whereConsolidated := " WHERE is_consolidated = " + s.dialect.BoolLit(true)
		if userID != "" {
			whereConsolidated += " AND user_id = ?"
		}
		err = s.queryRow("SELECT COUNT(*) FROM topics"+whereConsolidated, args...).Scan(&consolidatedCount)
		if err != nil {
	...
	// 2. Facts Stats
	err = s.queryRow("SELECT COUNT(*) FROM structured_facts"+whereUser, args...).Scan(&stats.TotalFacts)
	if err != nil {
		return nil, fmt.Errorf("failed to count facts: %w", err)
	}

	// Facts by Category
	rows, err := s.query("SELECT category, COUNT(*) FROM structured_facts"+whereUser+" GROUP BY category", args...)
	if err != nil {
	...
	// Facts by Type
	rowsType, err := s.query("SELECT type, COUNT(*) FROM structured_facts"+whereUser+" GROUP BY type", args...)
	if err != nil {
	...
	// 3. Messages Stats
	err = s.queryRow("SELECT COUNT(*) FROM history"+whereUser, args...).Scan(&stats.TotalMessages)
	if err != nil {
	...
	err = s.queryRow("SELECT COUNT(*) FROM history"+whereUnprocessed, args...).Scan(&stats.UnprocessedMessages)
	if err != nil {
		return nil, fmt.Errorf("failed to count unprocessed messages: %w", err)
	}

	// 4. RAG Stats
	err = s.queryRow("SELECT COUNT(*), COALESCE(AVG(total_cost_usd), 0) FROM rag_logs"+whereUser, args...).Scan(&stats.TotalRAGQueries, &stats.AvgRAGCost)
	if err != nil {
		return nil, fmt.Errorf("failed to get rag stats: %w", err)
	}

	// 5. Activity (Messages per Day) - Last 14 days
	dayExpr := s.dialect.DateExpr("created_at")
	whereActivity := " WHERE " + s.dialect.SinceDaysPredicate("created_at", 14)
	if userID != "" {
		whereActivity += " AND user_id = ?"
	}

	queryActivity := "SELECT " + dayExpr + ", COUNT(*) FROM history" + whereActivity + " GROUP BY " + dayExpr + " ORDER BY " + dayExpr
	rowsActivity, err := s.query(queryActivity, args...)
	if err != nil {
	...
	whereFactsGrowth := " WHERE " + s.dialect.SinceDaysPredicate("created_at", 30)
	if userID != "" {
		whereFactsGrowth += " AND user_id = ?"
	}

	queryFactsGrowth := "SELECT " + dayExpr + ", COUNT(*) FROM structured_facts" + whereFactsGrowth + " GROUP BY " + dayExpr + " ORDER BY " + dayExpr
	rowsFactsGrowth, err := s.query(queryFactsGrowth, args...)
	if err != nil {
