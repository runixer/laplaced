...
func TestPerson_ExternalIDRoundTrip(t *testing.T) {
	...
	scope := ScopeID("2")
	tr, ext := "mattermost", "u26charstring"
	...
	p, err := store.FindPersonByExternalID(scope, "mattermost", "u26charstring")
	require.NoError(t, err)
	require.NotNil(t, p)
	assert.Equal(t, "Alice (@alice)", p.DisplayName)

	// Scope isolation and wrong id both miss.
	other, err := store.FindPersonByExternalID(ScopeID("99"), "mattermost", "u26charstring")
	require.NoError(t, err)
	assert.Nil(t, other)
	miss, err := store.FindPersonByExternalID(scope, "mattermost", "nobody")
	require.NoError(t, err)
	assert.Nil(t, miss)
}
