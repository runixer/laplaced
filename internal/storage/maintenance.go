...
// GetDBSize returns the size of the database in bytes. SQLite uses the file
// size; Postgres uses pg_database_size(current_database()).
func (s *Store) GetDBSize() (int64, error) {
	if s.dialect.Name() == "postgres" {
		var size int64
		err := s.queryRow("SELECT pg_database_size(current_database())").Scan(&size)
		return size, err
	}
...
// GetTableSizes returns the size of each table in bytes. SQLite uses the dbstat
// virtual table; Postgres uses pg_total_relation_size over public tables.
...
func (s *Store) CountAgentLogs() (int64, error) {
	var count int64
	err := s.queryRow("SELECT COUNT(*) FROM agent_logs").Scan(&count)
	return count, err
}
...
func (s *Store) CountFactHistory() (int64, error) {
	var count int64
	err := s.queryRow("SELECT COUNT(*) FROM fact_history").Scan(&count)
	return count, err
}
