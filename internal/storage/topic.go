...
func (s *Store) AddTopic(topic Topic) (int64, error) {
	...
	_, err = s.exec(updateQuery, id, topic.UserID, topic.StartMsgID, topic.EndMsgID)
	return id, err
}
...
func (s *Store) DeleteTopicCascade(userID ScopeID, id int64) error {
	...
	if _, err := tx.Exec(s.rebind("DELETE FROM structured_facts WHERE user_id = ? AND topic_id = ?"), userID, id); err != nil {
		_ = tx.Rollback()
		return err
	}

	// Delete history linked to this topic (no user_id in fact_history table)
	if _, err := tx.Exec(s.rebind("DELETE FROM fact_history WHERE topic_id = ?"), id); err != nil {
		_ = tx.Rollback()
		return err
	}

	// Delete topic (filtered by user_id)
	if _, err := tx.Exec(s.rebind("DELETE FROM topics WHERE id = ? AND user_id = ?"), id, userID); err != nil {
		_ = tx.Rollback()
		return err
	}
...
func (s *Store) GetLastTopicEndMessageID(userID ScopeID) (int64, error) {
	...
	err := s.queryRow(query, userID).Scan(&maxID)
	if err != nil {
		return 0, err
	}
	if maxID.Valid {
...
func (s *Store) SetTopicFactsExtracted(userID ScopeID, topicID int64, extracted bool) error {
	...
	_, err := s.exec(query, extracted, topicID, userID)
	return err
}
...
func (s *Store) SetTopicConsolidationChecked(userID ScopeID, topicID int64, checked bool) error {
	...
	_, err := s.exec(query, checked, topicID, userID)
	return err
}

func (s *Store) GetTopicsPendingFacts(userID ScopeID) ([]Topic, error) {
	query := "SELECT id, user_id, summary, start_msg_id, end_msg_id, size_chars, embedding, facts_extracted, is_consolidated, consolidation_checked, created_at FROM topics WHERE user_id = ? AND facts_extracted = " + s.dialect.BoolLit(false) + " ORDER BY created_at ASC, id ASC"
	rows, err := s.query(query, userID)
	if err != nil {
		return nil, err
	}
...
func (s *Store) GetMergeCandidates(userID ScopeID) ([]MergeCandidate, error) {
	...
	query := fmt.Sprintf(`
		...
		WHERE t1.user_id = ?
		  AND t1.id < t2.id
		  AND t1.consolidation_checked = %[1]s
		  AND t2.consolidation_checked = %[1]s
		  AND (t2.start_msg_id - t1.end_msg_id) < 100
		  AND (t2.start_msg_id - t1.end_msg_id) > 0
		LIMIT 50
	`, s.dialect.BoolLit(false))
